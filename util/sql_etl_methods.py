...
def merge_dfs_tables(table_name, business_key_col, dataframe, db_context):
    ...
    if not elements_to_update.empty:
        existing_elements = pd.read_sql_query(
            'SELECT * FROM {table_name} WHERE surr_id IN ({ids})'.format(table_name=table_name, ids=','.join(elements_to_update['surr_id'].astype(str))), db_context)
...
def etl_code(con_db_stg):
    ...
    etl_df.to_sql("etl_proc", con_db_stg, if_exists="append", index=False)
    ...
    get_etl_code = etl_cod["cod_etl"][0]
